...
sbatch --job-name=myjob --nodes=1 --ntasks=1 --cpus-per-task=64 --mem=250G --time=002:00:00 --partition=calea --wrap="srun -n 1 env OMP_PLACES=threads env OMP_PROC_BIND=true ./main.out 9"
sbatch --job-name=myjob --nodes=1 --ntasks=1 --cpus-per-task=64 --mem=250G --time=002:00:00 --partition=calea --wrap="srun -n 1 env OMP_PLACES=threads env OMP_PROC_BIND=true ./main.out 10"
sbatch --job-name=myjob --nodes=1 --ntasks=1 --cpus-per-task=64 --mem=250G --time=002:00:00 --partition=calea --wrap="srun -n 1 env OMP_PLACES=threads env OMP_PROC_BIND=true ./main.out 11"

...
void TestLookupTable()
{
    ...
    for (int i = 0; i < 9; i++)
    {
        double phi = 2.0 * M_PI * i / 8.0;
        table.Add(phi, sin(phi));
    }

    cout << table.Evaluate(0.0 * M_PI) << endl;
    cout << table.Evaluate(0.5 * M_PI) << endl;
    cout << table.Evaluate(1.0 * M_PI) << endl;
    cout << table.Evaluate(1.5 * M_PI) << endl;
    cout << table.Evaluate(2.0 * M_PI) << endl;

    table.WriteToCsv("table", 100);
}

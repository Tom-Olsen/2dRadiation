...
void Quadrature(const Stencil& stencil)
{
    ...
    cout << phi/M_PI << "pi, " << value << ", " << Fourier::GetValue(phi,coefficients,stencil.nCoefficients) << endl << endl;
...
void Interpolation(const Stencil& stencil)
{
    ...
    for(size_t i=0; i<m; i++)
    {
        ...
        {
            value0 += data[neighbourIndexes0[j]] * neighbourWeights0[j];
            value1 += data[neighbourIndexes1[j]] * neighbourWeights1[j];
        }
...
int main()
{
    ...
    // Stencil(15,5).interpolationGrid.Print();
    // Stencil(20,10).Print();

...
// Constructors:
Grid::Grid(size_t nx, size_t ny, Coord start, Coord end) :
nx(nx), ny(ny), startx(start[1]), starty(start[2]), endx(end[1]), endy(end[2])
{
    if(end[1] < start[1])
...
nx(grid.nx), ny(grid.ny), startx(grid.startx), starty(grid.starty), endx(grid.endx), endy(grid.endy)
{
    dx = (endx - startx) / (nx - 1);
    dy = (endy - starty) / (ny - 1);
    dt = m_cfl * std::min(dx, dy);
 ...
 const int frameNumber, std::string directory, std::string name)
{
    ...
    name = (name == "") ? "data" :  name;
    std::ofstream fileOut(directory + name + "_" + FrameNumber(frameNumber) + "_" + std::to_string(nx) + "x" + std::to_string(ny) + "y" + ".csv");

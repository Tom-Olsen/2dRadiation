...
using Double4Buffer = std::vector<std::array<double, 4>, AlignedArrayAllocator<std::array<double, 4>>>;
...
inline void Marker(std::string name = "", bool newline = false)
{
    static int i = 0;
...
inline void PrintList(ListOfT list, std::string name)
{
    std::cout << name << ": [" << list[0];
    for (auto it = std::next(begin(list)); it != end(list); it++)
        std::cout << "|" << *it;
    std::cout << "]\n";
}
